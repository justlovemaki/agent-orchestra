# PROJECT_EVOLUTION.md - Agent Orchestra

> 项目定位：OpenClaw Agent 可视化调度中枢
> 维护方式：战略文档，非 changelog。供 `git-project-evolution` 每轮读取与更新。

---

## 1. 项目目标

Agent Orchestra 的最终目标是成为 OpenClaw 生态的**可视化调度中枢**：

- 实时掌握所有 Agent 状态、任务进度、系统健康
- 可派发、编排、监控多 Agent 协作任务
- 提供数据驱动的决策支持（趋势分析、负载洞察、智能推荐）
- 最终从「管理面板」演进为「可扩展的 Agent 平台」

---

## 2. 当前成熟度阶段

**Stage 4: 产品化期**

原因：
- 核心功能（Agent 管理、任务派发、会话控制、工作流编排）全部完成
- 辅助系统（备份、通知、图表、推荐、用户组）已高度丰富
- 本轮完成了 Docker 化部署，部署维度从「无」跃迁为「生产就绪」
- 下一步应继续收敛：API 文档、代码架构拆分、自动化测试

---

## 3. 能力矩阵

| 能力维度 | 状态 | 说明 |
|---------|------|------|
| **Agent 状态监控** | ✅ 完成 | 总览、实时数据接入、SSE 日志流 |
| **任务管理** | ✅ 完成 | 创建、重试、取消、暂停/恢复、重新指派、批量操作、筛选视图 |
| **会话管理** | ✅ 完成 | 列表、历史、发送消息、启动子 Agent |
| **工作流编排** | ✅ 完成 | 创建、执行、步骤管理、Agent 推荐、实时推荐 |
| **数据可视化** | ✅ 完成 | 任务趋势、Agent 使用率、状态分布、负载占比、图表交互与筛选联动 |
| **通知系统** | ✅ 完成 | 渠道管理、模板、优先级/降级、健康检查、免打扰、统计图表 |
| **备份系统** | ✅ 完成 | 全量/增量、云存储、定时、版本控制、恢复 |
| **Agent 组合** | ✅ 完成 | CRUD、收藏、导入导出、使用分析、智能推荐、反馈学习、团队共享 |
| **用户与权限** | ✅ 完成 | 用户管理、用户组、角色控制 |
| **UI/UX** | ✅ 完成 | 响应式、深色主题、移动端适配、微交互 |
| **部署/容器化** | ✅ 完成 | Dockerfile 多阶段构建、docker-compose、健康检查、一键部署 |
| **健康检查 API** | ✅ 完成 | GET /api/health，含 data_dir/memory 检查，200/503 状态码 |
| **插件/扩展系统** | ❌ 未开始 | 无插件机制，所有功能硬编码 |
| **API 文档** | ❌ 未开始 | 无 OpenAPI/Swagger 文档 |
| **自动化测试** | ❌ 未开始 | 仅有 verify 脚本，无单元/集成测试 |
| **性能优化** | ❌ 未开始 | 无分页优化、无懒加载、无缓存策略 |

---

## 4. 架构概述

```
项目结构：Node.js 原生 HTTP 单体应用
├── server.js          — 主入口，HTTP 路由 + 所有 API
├── lib/               — 业务模块（23 个文件）
│   ├── runtime-utils.js     — OpenClaw API 交互
│   ├── workflows.js         — 工作流核心
│   ├── workflow-runner.js   — 工作流执行引擎
│   ├── notification-*.js    — 通知子系统（6 个文件）
│   ├── scheduled-backup*.js — 备份子系统（3 个文件）
│   ├── combination-*.js     — 组合推荐子系统
│   └── ...
├── public/            — 前端（单 HTML + app.js + styles.css）
├── data/              — JSON 文件持久化
├── Dockerfile         — 多阶段构建，Node.js 20 Alpine
├── docker-compose.yml — 一键部署配置
└── DEPLOY.md          — 部署文档
```

关键问题：server.js 路由过于集中（~6000行），lib/ 模块间耦合不明确，前端为单文件 SPA。

---

## 5. 已知问题与技术债

1. **server.js 臃肿** — 所有路由集中在一个文件（~6000行），应拆分为路由模块
2. **无测试覆盖** — 没有任何自动化测试，每次修改靠人工验证
3. **前端单文件** — public/app.js 承载了所有 UI 逻辑（~9500行），难以维护
4. **JSON 文件存储** — data/ 目录用 JSON 做持久化，并发安全和性能都有隐患
5. **无 API 文档** — 端点众多但无文档，外部集成困难

---

## 6. 演进路线图

### 近期（1-3 轮）— 产品化收敛
- **API 文档自动生成**：基于现有路由生成 OpenAPI 文档
- **代码架构拆分**：server.js 路由模块化
- **自动化测试框架**：核心 API 端点的集成测试

### 中期（4-10 轮）— 平台化跃迁
- **插件系统**：允许通过插件扩展新面板、新通知渠道、新数据源
- **性能优化**：大数据量下的分页、懒加载、缓存
- **多用户认证**：真实的登录/鉴权系统
- **前端模块化**：拆分 app.js 为组件

### 远期愿景
- 成为 OpenClaw 的官方可视化管理面板
- 支持插件市场，社区可贡献面板组件
- 从「管理工具」变成「Agent 协作平台」

---

## 7. 近期进化记录（最近 5 轮）

| 轮次 | 日期 | 类型 | 改动概要 | 阶段变化 |
|------|------|------|---------|---------|
| 最新 | 2026-03-25 | 🚀 大演进 | Docker化部署 + 健康检查API + 部署文档 | 不变（Stage 4 内部维度跃迁） |
| -1 | 2026-03-25 | 🔧 中迭代 | 工作流步骤实时 Agent 推荐（前端） | 不变 |
| -2 | 2026-03-25 | 🔧 中迭代 | 工作流 Agent 推荐后端 API | 不变 |
| -3 | 2026-03-25 | 🔧 中迭代 | 免打扰时段 + 通知优先级筛选 | 不变 |
| -4 | 2026-03-25 | 🔧 中迭代 | 通知渠道健康检查系统 | 不变 |
