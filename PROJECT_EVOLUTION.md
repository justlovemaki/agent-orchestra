...
3. **数据备份与恢复**：用户数据和预设的导入导出功能

---

### 本轮进化 (2026-03-19 12:42 PM)

#### 本轮目标
实现 **数据备份与恢复功能**，为系统提供完整的数据备份、恢复和灾难恢复能力，支持数据迁移和系统升级保护。

#### 为什么现在做这个
- **来源层级**：PROJECT_EVOLUTION.md 中"下一步建议"明确列为第三优先级，在 RBAC 权限系统完成后自然延伸
- **选择理由**：
  1. 随着用户系统、预设、模板、工作流等功能的完善，数据价值日益重要
  2. 系统升级或迁移时需要完整的数据备份机制
  3. 防止意外数据丢失，提供灾难恢复能力
  4. 与现有导出功能形成完整的数据管理闭环
  5. 实现成本适中，用户可感知价值高（数据安全）
- **暂不处理的方向**：
  - 更细粒度的权限体系：当前 RBAC 已足够支持备份恢复功能
  - SSO/OAuth 集成：备份功能是更基础的能力，优先级更高

#### 实现内容
1. **后端 API** (`server.js`):
   - `GET /api/admin/backup?mode=full|incremental`：创建数据备份
     - 完整备份：导出所有数据
     - 增量备份：仅导出最近 7 天数据
     - 排除临时数据（id 以 temp- 开头的任务）
     - 返回 JSON 文件供下载
   - `POST /api/admin/restore`：恢复数据备份
     - 支持合并模式（保留现有 + 添加新的）
     - 支持覆盖模式（清空现有，完全使用备份数据）
     - 恢复前自动创建当前状态快照
     - 验证备份文件格式和版本
   - `GET /api/admin/backup/status`：获取备份状态信息
     - 上次备份时间、备份文件大小、备份次数

2. **备份数据结构** (`server.js`):
   - 备份格式：`{ version, backupAt, backupMode, data }`
   - 备份内容包括：users、tasks、templates、agent-groups、shared-presets、user-presets、user-templates、workflows、workflow-runs、audit-events
   - 密码数据保持哈希状态，不暴露明文
   - tokens 不包含在备份中（安全考虑）

3. **审计集成** (`lib/audit.js`):
   - 新增审计事件类型：`backup.created`、`backup.restored`、`backup.auto_snapshot_created`
   - 记录备份模式、文件大小、恢复结果等详细信息

4. **前端功能** (`public/app.js`, `public/index.html`, `public/styles.css`):
   - 用户管理面板新增"数据备份与恢复"按钮（仅管理员可见）
   - 备份模态框包含：
     - 备份状态显示（上次备份时间、大小、次数）
     - 创建备份区域（完整/增量模式选择）
     - 恢复数据区域（文件上传、模式选择、自动快照选项）
     - 备份历史列表（从审计日志读取）
   - 支持拖拽上传备份文件
   - 备份文件预览（显示备份时间、模式、数据摘要）
   - 恢复前确认对话框（说明恢复模式和影响范围）
...
- ✅ 新增 885 行代码（server.js +331, app.js +255, index.html +89, styles.css +174, audit.js +5, PROJECT_EVOLUTION.md +31）
- ✅ Git 提交成功：`1d69f59 feat: add data backup and restore functionality for disaster recovery`
- ✅ 代码已推送到远端
...
- **更新区块**：
  - "当前结论" → 添加数据备份与恢复功能描述
  - "当前已完成" → 添加数据备份与恢复功能详细说明
  - "管理员 API" → 添加备份恢复相关接口说明
  - 新增本轮进化记录

#### GitHub 同步结果
- **提交**：`1d69f59 feat: add data backup and restore functionality for disaster recovery`
- **推送**：已成功推送到 `main` 分支
- **项目地址**：https://github.com/justlovemaki/agent-orchestra
- **Issue 关闭**：无 open issues，本轮属于主动进化
...
1. **更细粒度的权限体系**：如预设级别/模板级别的单独权限、权限继承
2. **更多企业级功能**：用户组/部门管理、SSO/OAuth 集成、操作日志详情
3. **定时自动备份**：支持配置定时任务，自动创建数据备份

---

### 本轮进化 (2026-03-19 18:42 PM)

#### 本轮目标
实现 **定时自动备份功能**，在手动备份基础上增加按日/周自动执行备份的能力，支持可配置的执行时间、备份模式、保留数量，实现零运维成本的数据保护。

#### 为什么现在做这个
- **来源层级**：PROJECT_EVOLUTION.md 中"下一步建议"明确列为第三优先级，在数据备份与恢复功能完成后自然延伸
- **选择理由**：
  1. 手动备份需要用户主动操作，容易遗忘，自动备份可提供持续保障
  2. 已有数据备份 API 基础，扩展成本低
  3. 用户可感知价值高（零运维成本、自动数据保护）
  4. 与企业级数据保护实践对齐（定期自动备份是标准做法）
  5. 实现成本适中，新增独立调度模块即可
- **暂不处理的方向**：
  - 更细粒度的权限体系：当前 RBAC 已足够支持备份功能
  - SSO/OAuth 集成：自动备份是更基础的能力，优先级更高

#### 实现内容
1. **新增模块** (`lib/scheduled-backup.js`):
   - 独立定时备份调度模块，283 行代码
   - 支持每日/每周两种频率配置
   - 可配置执行时间（HH:MM 格式）
   - 每周备份可配置星期几（0-6）
   - 可配置保留数量（1-100），自动清理过期备份
   - 配置持久化至 `data/scheduled-backup-config.json`
   - 历史记录持久化至 `data/scheduled-backup-history.json`
   - 提供 `executeScheduledBackup()` 执行备份
   - 提供 `startScheduler()` / `stopScheduler()` 控制调度器
   - 提供 `getConfig()` / `updateConfig()` 管理配置

2. **后端 API** (`server.js`):
   - `GET /api/admin/scheduled-backup/config`：获取定时备份配置（含下次执行时间）
   - `PUT /api/admin/scheduled-backup/config`：更新定时备份配置（仅管理员）
     - 支持启用/禁用、频率、时间、星期、模式、保留数量
     - 配置变更时自动重启调度器
     - 记录审计事件 `scheduled_backup.config_changed`
   - `GET /api/admin/scheduled-backup/history`：获取自动备份历史记录（最多 50 条）
   - `POST /api/admin/scheduled-backup/run`：手动触发一次自动备份
     - 记录审计事件 `scheduled_backup.executed` / `scheduled_backup.failed`

3. **审计集成** (`lib/audit.js`):
   - 新增审计事件类型：`scheduled_backup.config_changed`、`scheduled_backup.executed`、`scheduled_backup.failed`
   - 记录配置变更者、执行结果、错误信息、文件大小

4. **前端功能** (`public/app.js`, `public/index.html`, `public/styles.css`):
   - 备份模态框新增"定时备份配置"区域
   - 支持启用/禁用定时备份（开关控件）
   - 支持配置频率（每日/每周下拉选择）
   - 支持配置执行时间（时间选择器，HH:MM 格式）
   - 每周频率时显示星期选择（周日 - 周六）
   - 支持配置备份模式（完整/增量）
   - 支持配置保留数量（1-100 数字输入）
   - 显示下次自动备份时间（动态更新）
   - 支持"立即执行一次"按钮
   - 显示自动备份历史记录列表（成功/失败状态、时间、文件大小、错误信息）
   - UI 状态管理：`loadScheduledBackupConfig()`、`updateDayOfWeekVisibility()`、`loadScheduledBackupHistory()`

5. **文档更新** (`PROJECT_EVOLUTION.md`):
   - 更新"当前结论"部分，添加定时自动备份功能描述
   - 更新"当前已完成"部分，添加定时自动备份详细说明
   - 更新"一句话结论"
   - 新增本轮进化记录

#### 验证结果
- ✅ 代码语法检查通过 (`node --check lib/scheduled-backup.js`, `node --check server.js`, `node --check public/app.js`)
- ✅ 新增 701 行代码（lib/scheduled-backup.js 新建 283 行，server.js +138, app.js +147, index.html +78, styles.css +51, audit.js +5）
- ✅ Git 提交成功：`1e98c54 feat: add scheduled automatic backup with configurable frequency and retention`
- ✅ 代码已推送到远端
...
- **更新区块**：
  - "当前结论" → 添加定时自动备份功能描述
  - "当前已完成" → 添加定时自动备份功能详细说明
  - "一句话结论" → 更新功能说明，标记定时自动备份已完成
  - 新增本轮进化记录

#### GitHub 同步结果
- **提交**：`1e98c54 feat: add scheduled automatic backup with configurable frequency and retention`
- **推送**：已成功推送到 `main` 分支
- **项目地址**：https://github.com/justlovemaki/agent-orchestra
- **Issue 关闭**：无 open issues，本轮属于主动进化
...
### 本轮进化 (2026-03-19 21:42 PM)

#### 本轮目标
实现 **备份通知功能**，在定时自动备份执行完成或失败时，自动发送通知消息到配置的 IM 渠道（飞书/钉钉/企业微信/Slack），使用户无需查看面板即可知晓备份状态。

#### 为什么现在做这个
- **来源层级**：PROJECT_EVOLUTION.md 中"下一步建议"明确列为第三优先级，在定时自动备份功能完成后自然延伸
- **选择理由**：
  1. 直接复用现有的 IM 分享基础设施（飞书/钉钉/企业微信/Slack API），实现成本低
  2. 用户可感知价值高（备份状态主动通知，无需被动查看）
  3. 与企业级运维实践对齐（关键操作结果通知是标准做法）
  4. 完善备份功能闭环（配置→执行→通知→审计）
  5. 实现成本适中，新增通知发送逻辑即可
- **暂不处理的方向**：
  - 更细粒度的权限体系：当前 RBAC 已足够支持备份功能
  - 云存储后端：备份通知是更基础的能力，优先级更高

#### 实现内容
1. **审计事件类型** (`lib/audit.js`):
   - 新增 `scheduled_backup.notified` 事件类型
   - 记录通知发送结果（成功渠道、失败渠道、错误信息）

2. **定时备份模块升级** (`lib/scheduled-backup.js`):
   - 配置结构新增 `notification: { enabled: boolean, channels: string[] }`
   - 新增 `setNotificationFunction(fn)` 导出函数
   - `executeScheduledBackup()` 在备份成功/失败后自动调用通知函数
   - 通知参数包含：success、type、fileName、fileSize、duration、error、channels

3. **后端 API 升级** (`server.js`):
   - 新增 `sendFeishuTextMessage()`、`sendDingTalkTextMessage()`、`sendWecomTextMessage()`、`sendSlackTextMessage()` 函数
   - 新增 `formatBytes()` 工具函数
   - 新增 `handleScheduledBackupNotification()` 统一处理通知发送
   - 通知消息格式包含：备份类型、时间、状态、文件名、大小、耗时/错误信息
   - PUT `/api/admin/scheduled-backup/config` 支持 `notification` 配置验证
   - 自动记录 `scheduled_backup.notified` 审计事件

4. **前端功能** (`public/index.html`, `public/app.js`, `public/styles.css`):
   - 备份模态框新增"启用备份通知"开关
   - 新增通知渠道选择（飞书/钉钉/企业微信/Slack 复选框）
   - 加载配置时自动读取通知设置
   - 保存配置时自动同步通知设置
   - 新增 `.scheduled-backup-notification`、`.notification-channels` 样式

5. **通知消息示例**:
   ```
   📦 Agent Orchestra 定时备份通知
   ━━━━━━━━━━━━━━━━━━━━
   ⏰ 时间：2026/3/19 10:00:00
   📋 类型：完整备份
   📊 状态：成功
   📁 文件：scheduled-backup-2026-03-19T10-00-00.000Z.json
   💾 大小：1.5 MB
   ⏱️ 耗时：12.3 秒
   ━━━━━━━━━━━━━━━━━━━━
   ```
...
#### 验证结果
- ✅ 代码语法检查通过 (`node -c server.js`, `node -c lib/scheduled-backup.js`, `node -c lib/audit.js`)
- ✅ 新增 357 行代码（server.js +167, scheduled-backup.js +40, app.js +50, index.html +29, styles.css +24, audit.js +2）
- ✅ Git 提交成功：`6765e36 feat: add backup notification to IM channels (Feishu/DingTalk/WeCom/Slack)`
- ✅ 代码已推送到远端
...
- **更新区块**：
  - "当前结论" → 添加备份通知功能描述
  - "当前已完成" → 添加备份通知功能详细说明
  - "一句话结论" → 更新功能说明，标记备份通知已完成
  - "下一步建议" → 移除"备份通知功能"，新增"备份存储优化"
  - 新增本轮进化记录

#### GitHub 同步结果
- **提交**：`6765e36 feat: add backup notification to IM channels (Feishu/DingTalk/WeCom/Slack)`
- **推送**：已成功推送到 `main` 分支
- **项目地址**：https://github.com/justlovemaki/agent-orchestra
- **Issue 关闭**：无 open issues，本轮属于主动进化
...
1. **更细粒度的权限体系**：如预设级别/模板级别的单独权限、权限继承
2. **更多企业级功能**：用户组/部门管理、SSO/OAuth 集成、操作日志详情
3. **备份存储优化**：~~支持 S3/OSS 等云存储后端~~ ✅ 已完成

---

### 本轮进化 (2026-03-20 00:42 AM)

#### 本轮目标
实现 **云存储备份功能**，支持将备份文件上传到 S3 兼容存储（阿里云 OSS / AWS S3 / MinIO），解决本地磁盘空间限制问题，支持云端备份管理和恢复。

#### 为什么现在做这个
- **来源层级**：PROJECT_EVOLUTION.md 中"下一步建议"明确列为第三优先级，在备份通知功能完成后自然延伸
- **选择理由**：
  1. 本地备份受磁盘空间限制，云存储可提供近乎无限的存储容量
  2. 云端备份支持异地容灾，提升数据安全性
  3. 已有备份功能基础，扩展成本低
  4. 用户可感知价值高（数据安全保障、跨设备恢复）
  5. 与企业级数据保护实践对齐（云端备份是标准做法）
- **暂不处理的方向**：
  - 更细粒度的权限体系：当前 RBAC 已足够支持备份功能
  - SSO/OAuth 集成：云存储备份是更基础的能力，优先级更高

#### 实现内容
1. **新增云存储模块** (`lib/cloud-storage.js`):
   - 基于 @aws-sdk/client-s3 实现 S3 兼容客户端
   - 支持阿里云 OSS、AWS S3、MinIO 等 S3 兼容存储
   - 提供上传、下载、列出、删除文件的统一接口
   - 配置持久化至 `data/cloud-storage-config.json`
   - 敏感信息（accessKeySecret）仅在配置时传输，读取时不返回

2. **审计事件类型扩展** (`lib/audit.js`):
   - 新增 `cloud_storage.config_changed`：云存储配置变更
   - 新增 `cloud_backup.uploaded`：备份上传到云端
   - 新增 `cloud_backup.downloaded`：从云端下载备份
   - 新增 `cloud_backup.deleted`：删除云端备份
   - 新增 `cloud_backup.restored`：从云端备份恢复

3. **定时备份模块升级** (`lib/scheduled-backup.js`):
   - 新增 `setCloudUploadFunction(fn)` 导出函数
   - 定时备份成功后自动上传到云存储（如果配置启用）
   - 记录云上传状态到历史记录

4. **后端 API** (`server.js`):
   - `GET/POST /api/admin/cloud-storage/config`：云存储配置管理
     - 配置项：provider (oss/s3/minio), bucket, region, endpoint, accessKeyId, accessKeySecret, enabled
   - `POST /api/admin/backup/cloud`：创建备份并上传到云存储
   - `GET /api/admin/backup/cloud/list`：列出云端备份文件
   - `POST /api/admin/backup/cloud/restore`：从云存储恢复备份
   - `POST /api/admin/backup/cloud/test`：测试云存储连接
   - 新增 `createBackupData()` 和 `performRestore()` 辅助函数

5. **前端功能** (`public/index.html`, `public/app.js`, `public/styles.css`):
   - 备份模态框新增"云存储配置"区域
   - 支持配置云存储提供商、存储桶、区域、凭证
   - 支持测试连接功能
   - 新增"云端备份管理"区域
   - 显示云端备份列表（文件名、大小、上传时间）
   - 支持"创建并上传云备份"功能
   - 支持从云端备份恢复（带确认对话框）
   - 新增云存储相关样式

6. **依赖更新** (`package.json`):
   - 新增 `@aws-sdk/client-s3` 依赖

#### 验证结果
- ✅ 代码语法检查通过 (`node --check lib/cloud-storage.js`, `node --check server.js`, `node --check public/app.js`)
- ✅ 新增约 1000+ 行代码（lib/cloud-storage.js 新建 230 行，server.js +350, app.js +350, index.html +80, styles.css +60, audit.js +5, scheduled-backup.js +10）
- ✅ Git 提交准备就绪
