...
const scheduledBackupNotifier = require('./scheduled-backup-notifier');
...
async function executeScheduledBackup() {
  ...
  try {
    ...
    try {
      const notifyResult = await scheduledBackupNotifier.sendScheduledBackupNotification({
        id: entry.id,
        success: true,
        type: config.mode || 'full',
        fileName: backupFileName,
      ...
      if (!notifyResult.skipped) {
        console.log(`[ScheduledBackup] 通知已发送: ${notifyResult.sent} 成功, ${notifyResult.failed} 失败`);
      }
    } catch (notifyError) {
      console.error('[ScheduledBackup] 发送通知失败:', notifyError.message);
    }
