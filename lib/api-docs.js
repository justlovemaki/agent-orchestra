...
function getOpenAPISpec(port) {
  return {
    ...
    tags: [
      ...
      { name: 'Performance', description: '性能监控（API 响应时间、内存使用、请求速率、错误率）' },
    ...
    paths: {
      ...
      '/api/metrics': {
        get: {
          tags: ['Performance'],
          summary: '获取性能指标',
          description: '获取 API 性能指标，包括响应时间、请求速率、错误率、内存使用等',
          operationId: 'getMetrics',
          parameters: [
            queryParam('timeRange', 'string', '时间范围 (1h, 24h, 7d, 1m)', '1h')
          ],
          responses: {
            '200': { description: '性能指标数据', content: jsonContent({ type: 'object' }) },
            '400': { description: '无效的时间范围参数' }
          }
        }
      },
      '/api/metrics/slow-requests': {
        get: {
          tags: ['Performance'],
          summary: '获取慢请求列表',
          description: '获取超过阈值的慢请求列表',
          operationId: 'getSlowRequests',
          parameters: [
            queryParam('timeRange', 'string', '时间范围 (1h, 24h, 7d, 1m)', '1h'),
            queryParam('limit', 'integer', '返回结果数量限制', '50'),
            queryParam('threshold', 'integer', '慢请求阈值（毫秒）', '1000')
          ],
          responses: {
            '200': { description: '慢请求列表', content: jsonContent({ type: 'object' }) },
      ...
      '/api/metrics/summary': {
        get: {
          tags: ['Performance'],
          summary: '获取性能摘要',
          description: '获取系统性能摘要信息，包括运行时间、请求统计、错误率等',
          operationId: 'getMetricsSummary',
          responses: { '200': { description: '性能摘要数据', content: jsonContent({ type: 'object' }) } }
        }
      },
