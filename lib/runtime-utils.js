...
const SIGKILL_WAIT_TIMEOUT_MS = 3000;
...
async function forceKillAndWait(pid, timeoutMs = SIGKILL_WAIT_TIMEOUT_MS, pollIntervalMs = STOP_POLL_INTERVAL_MS) {
  if (!Number.isInteger(pid) || pid <= 0) {
    return { killed: false, exited: true, timedOut: false };
  }
  ...
  while (true) {
    if (!isPidRunning(pid)) {
      return { killed: true, exited: true, timedOut: false };
    }
    if (Date.now() - startTime >= timeoutMs) {
      return { killed: true, exited: false, timedOut: true };
    }
    await new Promise(resolve => setTimeout(resolve, pollIntervalMs));
  }
}
