...
#### 筛选预设 / 收藏视图
- **默认快速预设**：内置“运行中任务”“高优先级失败任务”“今天创建的任务”三个一键视图
- **保存当前筛选**：可将当前筛选条件保存为命名预设，方便重复使用
- **快速应用 / 删除**：已保存视图支持一键切换与删除
- **预设重命名**：已保存视图可直接重命名，便于整理长期使用的筛选方案
- **顺序整理**：支持上移 / 下移已保存预设，让常用视图排在更靠前的位置
- **本地保存**：预设基于 localStorage 存储，无需后端改造，适合当前 MVP 阶段快速演进
