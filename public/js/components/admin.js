/**
 * Admin component - User and system management UI
 */

import { state } from '../../state.js';
import { fetchJson, loadWorkloadAlertConfig, saveWorkloadAlertConfig, loadWorkloadAlertHistory, loadNotificationChannels } from '../../api.js';
...
export function getAdminElements() {
  return {
    ...
    userGroupList: document.getElementById('userGroupList'),
    createUserGroupBtn: document.getElementById('createUserGroupBtn'),
    workloadAlertConfig: document.getElementById('workloadAlertConfig'),
    workloadAlertHistory: document.getElementById('workloadAlertHistory'),
    workloadAlertMsg: document.getElementById('workloadAlertMsg')
  };
}

export function renderAdminUI() {
  const { adminPanel, workloadAlertConfig } = getAdminElements();
  
  if (!adminPanel) return;
  
  if (state.isAdmin) {
    show(adminPanel);
    if (workloadAlertConfig) show(workloadAlertConfig);
    loadAllUsers();
    loadUserGroupsList();
    initUserGroupUI();
    ...
    hide(adminPanel);
    if (workloadAlertConfig) hide(workloadAlertConfig);
  }
}

export async function loadAllUsers() {
  const { userList } = getAdminElements();
  if (!userList) return;
...
export function renderUserManagementPanel() {
  const { userList } = getAdminElements();
  if (!userList) return;
  
  if (state.allUsers.length === 0) {
    userList.innerHTML = '<div class="muted small">暂无用户</div>';
    return;
  }
  ...
  userList.innerHTML = state.allUsers.map(user => {
    ...
    const groupName = user.groupId ? (allUserGroups.find(g => g.id === user.groupId)?.name || '未知组') : '';
    
    return `
      <div class="user-item" data-user-id="${user.id}">
        <div class="user-item-info">
          <div class="user-item-name">${escapeHtml(user.name)} ${isCurrentUser ? '<span class="muted">(本人)</span>' : ''} ${groupName ? `<span class="user-group-badge">${escapeHtml(groupName)}</span>` : ''}</div>
  ...
  userList.querySelectorAll('.update-role-btn').forEach(btn => {
    btn.addEventListener('click', async () => {
      const userId = btn.dataset.userId;
      const userName = btn.dataset.userName;
      const select = userList.querySelector(`.user-role-select[data-user-id="${userId}"]`);
  ...
  userList.querySelectorAll('.update-group-btn').forEach(btn => {
    btn.addEventListener('click', async () => {
      const userId = btn.dataset.userId;
      const userName = btn.dataset.userName;
      const select = userList.querySelector(`.user-group-select[data-user-id="${userId}"]`);
...
export async function loadUserGroupsList() {
  try {
...
export function renderUserGroupList() {
  const { userGroupList } = getAdminElements();
  if (!userGroupList) return;
  
  if (allUserGroups.length === 0) {
    userGroupList.innerHTML = '<div class="muted small">暂无用户组</div>';
    return;
  }
  
  userGroupList.innerHTML = allUserGroups.map(group => `
  ...
  userGroupList.querySelectorAll('.edit-group-btn').forEach(btn => {
    btn.addEventListener('click', () => openUserGroupModal(btn.dataset.groupId));
  });
  
  userGroupList.querySelectorAll('.delete-group-btn').forEach(btn => {
    btn.addEventListener('click', async () => {
...
export function initUserGroupUI() {
...
export function renderWorkloadAlertConfig() {
  ...
  if (thresholdInput) thresholdInput.value = config.threshold || 5;
  if (warningThresholdInput) warningThresholdInput.value = config.warningThreshold || 0.8;
  if (criticalThresholdInput) criticalThresholdInput.value = config.criticalThreshold || 1.0;
  if (silencePeriodInput) silencePeriodInput.value = config.silencePeriodMinutes || 30;
...
export function renderWorkloadAlertHistory() {
  const { workloadAlertHistory } = getAdminElements();
  if (!workloadAlertHistory) return;
  
  const history = state.workloadAlertHistory || [];
  
  if (history.length === 0) {
    workloadAlertHistory.innerHTML = '<div class="muted small">暂无预警记录</div>';
    return;
  }
  
  workloadAlertHistory.innerHTML = history.map(item => `
    <div class="workload-alert-item">
      <div class="workload-alert-info">
        <span class="workload-alert-level ${item.level}">${item.level === 'warning' ? '警告' : '严重'}</span>
        <span>${escapeHtml(item.agentName)}</span>
        <span>负载: ${item.workload}</span>
      </div>
      <div class="workload-alert-time muted small">${formatDate(item.triggeredAt)}</div>
    </div>
  `).join('');
}

function initChannelFormHandlers() {
  const saveBtn = document.getElementById('saveWorkloadAlertConfigBtn');
  if (saveBtn) {
    saveBtn.addEventListener('click', async () => {
      ...
      const payload = {
        ...
        threshold: parseInt(thresholdInput?.value || '5', 10),
        warningThreshold: parseFloat(warningThresholdInput?.value || '0.8'),
        criticalThreshold: parseFloat(criticalThresholdInput?.value || '1.0'),
        silencePeriodMinutes: parseInt(silencePeriodInput?.value || '30', 10),
        messageTemplate: {
      ...
      try {
        await saveWorkloadAlertConfig(payload);
        state.workloadAlertConfig = payload;
...
export function initAdminComponent() {
  return {
    renderAdminUI,
    loadAllUsers,
    loadUserGroupsList,
